...
describe('Country e2e test', () => {
  ...
  const countrySample = { name: 'but', code: 'af', phoneCode: 'or ch' };
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="name"]`).type('winged ack');
      cy.get(`[data-cy="name"]`).should('have.value', 'winged ack');

      cy.get(`[data-cy="code"]`).type('no');
      cy.get(`[data-cy="code"]`).should('have.value', 'no');

      cy.get(`[data-cy="phoneCode"]`).type('duh');
      cy.get(`[data-cy="phoneCode"]`).should('have.value', 'duh');
